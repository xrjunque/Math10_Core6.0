...
#Region "Parse and Evaluate"
    ...
    Sub CheckParenthesesMatched(s As String)
        ...
        End If
    End Sub
    Private Function PrepareOutput(r As Result) As String
        Dim s As String = ""
        Try
            If G10.mathml AndAlso G10.currBase <> Rational_1.base.Hexadecimal Then
                s = r.eMtx.ToString_eMtx(
                ExprMatrix.matrixOutputFormat.semicolonAndPipe, G10.nDec, G10.sImg, G10.CI)
                's = Replace(s, "*", "·")
                Dim vRow() As String = Split(s, "|")
                For i As Int32 = 0 To vRow.Length - 1
                    Dim vCol() As String = Split(vRow(i), ";")
                    For j As Int32 = 0 To vCol.Length - 1
                        vCol(j) = ConvertStringToMathML.ConvertToMathML(vCol(j))
                        If G10.currBase <> Rational_1.base.Decimal Then
            ...
            Else
                s = r.eMtx.ToString_eMtx(
                ExprMatrix.matrixOutputFormat.html, G10.nDec, G10.sImg, G10.CI)
    ...
    Public Function Evaluate(sMtx As String) As Result
        ...
        Try
            ...
            sExpr = Regex.Replace(sExpr, "@\s*(?<vars>[\w(),\d]+)\s*=\s*(?<val>[^@←]+)", "")
            sExpr = Regex.Replace(sExpr, "@\s*\|+", "")
                            ...
                            If vVars.Length = 1 Then
                                ...
                                For i As Int32 = 0 To vRoots.Length - 1
                                    If vRoots.Length > 1 Then
                                        r1.eMtx.Item(i, 0) = New Expression(vVars(0) + (i + 1).ToString + "=", 1)
                                    Else
            ...
            Do While Left(m.Value, 1) = "&"
                ...
                Else
                    Select Case LCase(m.Value)
                        ...
                        Case "&h" : G10.currBase = Rational_1.base.Hexadecimal
                            ChangePattern()
                        Case "&d" : G10.currBase = Rational_1.base.Decimal
                            ChangePattern()
                        Case "&o" : G10.currBase = Rational_1.base.Octal
                            ChangePattern()
                        Case "&b" : G10.currBase = Rational_1.base.Binary
                            ChangePattern()
                        Case "&j" : G10.sImg = "j"
                        Case "&i" : G10.sImg = "i"
